...
def fetch_and_append_missing_data(
    ...
    print(
        f"Fetching data from {datetime.fromtimestamp(start_timestamp, tz=timezone.utc)} to {datetime.fromtimestamp(end_timestamp, tz=timezone.utc)}"
    )
    ...
    for i, (chunk_start, chunk_end) in enumerate(time_chunks):
        print(
            f"Fetching chunk {i+1}/{len(time_chunks)}: {datetime.fromtimestamp(chunk_start, tz=timezone.utc)} to {datetime.fromtimestamp(chunk_end, tz=timezone.utc)}"
        )
        ...
        if chunk_data:
            ...
            df_chunk["timestamp"] = pd.to_numeric(
...
if __name__ == "__main__":
    ...
    output_filename = (
        existing_data_filename  # Output filename (same as the dataset name on Kaggle)
    )
